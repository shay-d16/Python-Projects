...
# catch if the user's clicks on th windo upper-right 'X' to ensure they want to close
def ask_quit(self):
...
def create_db(self):
    ...
    with conn:
        ...
        # You must comit() to save changes & close the databse connection
        conn.commit()
    conn.close()
    first_run(self)
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_email FROM tbl_phonebook WHERE col_fullness = (?)""", [value])
        varBody = cursor.fetchall()
        # This returns a tuple and we can slice it into 4 parts using date[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_lname = var_lname.strip() # This will ensure that the fisrt character of each word is capitalized
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,varlname)) # Combine our normalized names into a fullname
